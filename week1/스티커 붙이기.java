...
//1. 먼저 입력받음 (inputBoard)
//2. 왼쪽 위부터 오른쪽 아래까지 시뮬레이션(simulate)을 돌리다가 안되는 부분(checkBoard) 있으면 return(이미 노트북에 모눈종이가 채워져있는 영역을 만났다는 말)
    //2-1. 못찾았을 경우 회전시킴. 90도씩 네번 (roateBoard)
//3. 2번을 통과했으면(모눈종이가 들어갈 영역이 있다는 것을 확인) 노트북에 모눈종이를 채워 넣어줌 (fillnote)
//4. 순서대로 진행한 후 노트북에서 채워진 부분을 세면됨. (calculateAnswer)
